...
class RateLimitedLiteLLMModel(LiteLLMModel):
    ...
    # Load JSONs from the gemini directory adjacent to this agents package
    gem_llm_rates_path = os.path.join(os.path.dirname(current_dir), "gemini", "gem_llm_rates.json")
        ...
        **kwargs
        ...
        Args:
            ...
            model_info_path (str): Path to the JSON file containing model rate limit configurations.
                                   Defaults to the gem_llm_rates.json in utils/gemini.
        ...
        # Default to the gemini info file adjacent to this package if not provided
        self.model_info_path_for_fallback = model_info_path or os.path.join(os.path.dirname(self.current_dir), "gemini", "gem_llm_rates.json")
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        gem_llm_info_path = os.path.join(os.path.dirname(current_dir), "gemini", "gem_llm_info.json")
        ...
        try:
            ...
            if model_key not in model_info_json: 
                logger.warning(f"Model key '{model_key}' (from '{model_id}') not found in {self.model_info_path_for_fallback}. Using default limits.")
            ...
            logger.error(f"Error loading model info from {self.model_info_path_for_fallback}: {e}. Using fallback default limits.")
